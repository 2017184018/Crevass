...
void PlayerController::OnKeyPressed()
{
	...
	if (CommandCenter::GetApp()->m_StartJumpAnim || CommandCenter::GetApp()->m_StartAttackAnim || CommandCenter::GetApp()->m_StartFallAnim ||
		CommandCenter::GetApp()->m_StartSkillAnim || m_Owner->is_StartFallAnim) return;

	if (CommandCenter::GetApp()->IsSkillEnd) {
		m_Owner->IsSkill = false;
		CommandCenter::GetApp()->IsSkillEnd = false;
	}
	...
	case CameraType::Third:
		...
		if (InputHandler::IsKeyDown('A')) {
			...
			SoundManager::GetApp()->PlaySoundOnce(L"Attack.mp3", SoundManager::CHANNEL_ID::PLAYER_ATTACK, 3.0f);
		}
		...
		//	g_pFramework->m_pNetwork->Send(CS_PLAYER_MOVE);
			tmp = 0;
	//		SoundManager::GetApp()->PlaySoundOnce(L"Walking.mp3", SoundManager::CHANNEL_ID::PLAYER_RUNNING, 2.0f);
		}
		...
		if (InputHandler::IsKeyDown(VK_SPACE)) {
			...
			tmp = 8;
			SoundManager::GetApp()->PlaySoundOnce(L"Jump.mp3", SoundManager::CHANNEL_ID::PLAYER_JUMP, 2.5f);
		}
		else {
			//SpacePush = false;
		}
